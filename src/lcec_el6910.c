...
typedef struct {
  ...
  lcec_el6910_fsoe_crc_t *fsoe_crc;
} lcec_el6910_fsoe_t;
...
typedef struct {
  ...
  hal_bit_t *std_in_pins[LCEC_EL6910_DIO_MAX_COUNT];
  int std_in_count;
  unsigned int std_in_os;

  hal_bit_t *std_out_pins[LCEC_EL6910_DIO_MAX_COUNT];
  int std_out_count;
  ...
  lcec_el6910_fsoe_t fsoe[];
} lcec_el6910_data_t;

static const lcec_pindesc_t slave_pins[] = {
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_data_t, state), "%s.%s.%s.state" },
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_data_t, cycle_counter), "%s.%s.%s.cycle-counter" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};

static const lcec_pindesc_t fsoe_pins[] = {
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_t, fsoe_master_cmd), "%s.%s.%s.fsoe-%d-master-cmd" },
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_t, fsoe_master_connid), "%s.%s.%s.fsoe-%d-master-connid" },
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_t, fsoe_slave_cmd), "%s.%s.%s.fsoe-%d-slave-cmd" },
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_t, fsoe_slave_connid), "%s.%s.%s.fsoe-%d-slave-connid" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};

static const lcec_pindesc_t fsoe_crc_pins[] = {
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_crc_t, fsoe_master_crc), "%s.%s.%s.fsoe-%d-master-crc" },
  { HAL_U32, HAL_OUT, offsetof(lcec_el6910_fsoe_crc_t, fsoe_slave_crc), "%s.%s.%s.fsoe-%d-slave-crc" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};

void lcec_el6910_read(struct lcec_slave *slave, long period);
void lcec_el6910_write(struct lcec_slave *slave, long period);
...
int lcec_el6910_preinit(struct lcec_slave *slave) {
  ...
  slave->pdo_entry_count = LCEC_EL6910_PDOS;

  stdin_count = 0;
  stdout_count = 0;
  for (p = slave->modparams; p != NULL && p->id >= 0; p++) {
    switch(p->id) {
      case LCEC_EL6910_PARAM_SLAVEID:
        // find slave
        index = p->value.u32;
        ...
        slave->pdo_entry_count += LCEC_EL6910_PARAM_SLAVE_PDOS + LCEC_EL6910_PARAM_SLAVE_CH_PDOS * fsoeConf->data_channels;
        break;

      case LCEC_EL6910_PARAM_STDIN_NAME:
        stdin_count++;
        if (stdin_count > LCEC_EL6910_DIO_MAX_COUNT) {
      ...
      case LCEC_EL6910_PARAM_STDOUT_NAME:
        stdout_count++;
        if (stdout_count > LCEC_EL6910_DIO_MAX_COUNT) {
  ...
  if (stdin_count > 0) {
    slave->pdo_entry_count += LCEC_EL6910_STDIN_PDOS;
  }
  if (stdout_count > 0) {
    slave->pdo_entry_count += LCEC_EL6910_STDOUT_PDOS;
  }
...
int lcec_el6910_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  lcec_master_t *master = slave->master;
  lcec_el6910_data_t *hal_data;
  lcec_el6910_fsoe_t *fsoe_data;
  lcec_slave_modparam_t *p;
  int fsoe_idx, index, err;
  lcec_el6910_fsoe_crc_t *crc;
  struct lcec_slave *fsoe_slave;
  const LCEC_CONF_FSOE_T *fsoeConf;

  // initialize callbacks
  slave->proc_read = lcec_el6910_read;
  slave->proc_write = lcec_el6910_write;

  // count fsoe slaves
  for (fsoe_idx = 0, p = slave->modparams; p != NULL && p->id >= 0; p++) {
    if (p->id == LCEC_EL6910_PARAM_SLAVEID) {
      fsoe_idx++;
    }
  }

  // alloc hal memory
  if ((hal_data = hal_malloc(sizeof(lcec_el6910_data_t) + fsoe_idx * sizeof(lcec_el6910_fsoe_t))) == NULL) {
    rtapi_print_msg(RTAPI_MSG_ERR, LCEC_MSG_PFX "hal_malloc() for slave %s.%s failed\n", master->name, slave->name);
    return -EIO;
  }
  memset(hal_data, 0, sizeof(lcec_el6910_data_t));
  ...
  hal_data->std_in_count = export_std_pins(slave, pdo_entry_regs, LCEC_EL6910_PARAM_STDIN_NAME, hal_data->std_in_pins, HAL_IN);
  ...
  hal_data->std_out_count = export_std_pins(slave, pdo_entry_regs, LCEC_EL6910_PARAM_STDOUT_NAME, hal_data->std_out_pins, HAL_OUT);
  ...
  for (fsoe_idx = 0, fsoe_data = hal_data->fsoe, p = slave->modparams; p != NULL && p->id >= 0; p++) {
    if (p->id == LCEC_EL6910_PARAM_SLAVEID) {
      ...
      if ((fsoe_data->fsoe_crc = hal_malloc(fsoeConf->data_channels * sizeof(lcec_el6910_fsoe_crc_t))) == NULL) {
        rtapi_print_msg(RTAPI_MSG_ERR, LCEC_MSG_PFX "hal_malloc() for fsoe_slave %s.%s crc data failed\n", master->name, fsoe_slave->name);
        return -EIO;
      }
      memset(fsoe_data->fsoe_crc, 0, sizeof(lcec_el6910_fsoe_crc_t));
...
void lcec_el6910_read(struct lcec_slave *slave, long period) {
  lcec_master_t *master = slave->master;
  lcec_el6910_data_t *hal_data = (lcec_el6910_data_t *) slave->hal_data;
...
//TODO: printf("plc: %d %d %d %d %d\n", i, fsoe_data->fsoe_master_cmd_os, fsoe_data->fsoe_master_connid_os, fsoe_data->fsoe_slave_cmd_os, fsoe_data->fsoe_slave_connid_os);
