...
typedef struct {
  lcec_el30x4_chan_t chans[LCEC_EL30x4_CHANS];
} lcec_el30x4_data_t;

static const lcec_pindesc_t slave_pins[] = {
  { HAL_BIT, HAL_OUT, offsetof(lcec_el30x4_chan_t, overrange), "%s.%s.%s.ain-%d-overrange" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_el30x4_chan_t, underrange), "%s.%s.%s.ain-%d-underrange" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_el30x4_chan_t, error), "%s.%s.%s.ain-%d-error" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_el30x4_chan_t, sync_err), "%s.%s.%s.ain-%d-sync-err" },
  { HAL_S32, HAL_OUT, offsetof(lcec_el30x4_chan_t, raw_val), "%s.%s.%s.ain-%d-raw" },
  { HAL_FLOAT, HAL_OUT, offsetof(lcec_el30x4_chan_t, val), "%s.%s.%s.ain-%d-val" },
  { HAL_FLOAT, HAL_IO, offsetof(lcec_el30x4_chan_t, scale), "%s.%s.%s.ain-%d-scale" },
  { HAL_FLOAT, HAL_IO, offsetof(lcec_el30x4_chan_t, bias), "%s.%s.%s.ain-%d-bias" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};


static ec_pdo_entry_info_t lcec_el30x4_channel1[] = {
...
static ec_pdo_entry_info_t lcec_el30x4_channel2[] = {
...
static ec_pdo_entry_info_t lcec_el30x4_channel3[] = {
...
static ec_pdo_entry_info_t lcec_el30x4_channel4[] = {
...
static ec_pdo_info_t lcec_el30x4_pdos_in[] = {
    {0x1A00, 11, lcec_el30x4_channel1},
    {0x1A02, 11, lcec_el30x4_channel2},
    {0x1A04, 11, lcec_el30x4_channel3},
    {0x1A06, 11, lcec_el30x4_channel4},
};

static ec_sync_info_t lcec_el30x4_syncs[] = {
    {0, EC_DIR_OUTPUT, 0, NULL},
    {1, EC_DIR_INPUT,  0, NULL},
    {2, EC_DIR_OUTPUT, 0, NULL},
    {3, EC_DIR_INPUT,  4, lcec_el30x4_pdos_in},
    {0xff}
};

static void lcec_el30x4_read(struct lcec_slave *slave, long period);

static int lcec_el30x4_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  lcec_master_t *master = slave->master;
  lcec_el30x4_data_t *hal_data;
  lcec_el30x4_chan_t *chan;
  int i;
  int err;

  // initialize callbacks
  slave->proc_read = lcec_el30x4_read;

  // alloc hal memory
  if ((hal_data = hal_malloc(sizeof(lcec_el30x4_data_t))) == NULL) {
    rtapi_print_msg(RTAPI_MSG_ERR, LCEC_MSG_PFX "hal_malloc() for slave %s.%s failed\n", master->name, slave->name);
    return -EIO;
  }
  memset(hal_data, 0, sizeof(lcec_el30x4_data_t));
  slave->hal_data = hal_data;

  // initialize sync info
  slave->sync_info = lcec_el30x4_syncs;

  // initialize pins
  for (i=0; i<LCEC_EL30x4_CHANS; i++) {
...
static void lcec_el30x4_read(struct lcec_slave *slave, long period) {
  lcec_master_t *master = slave->master;
  lcec_el30x4_data_t *hal_data = (lcec_el30x4_data_t *) slave->hal_data;
  ...
  for (i=0; i<LCEC_EL30x4_CHANS; i++) {
    ...
    *(chan->raw_val) = value;
    *(chan->val) = *(chan->bias) + *(chan->scale) * (double)value * ((double)1/(double)0x7ff);
  }
}
