...
typedef struct {
  lcec_em37xx_chan_t chans[LCEC_EM37XX_CHANS];
} lcec_em37xx_data_t;

static const lcec_pindesc_t slave_pins[] = {
  { HAL_BIT, HAL_OUT, offsetof(lcec_em37xx_chan_t, overrange), "%s.%s.%s.press-%d-overrange" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_em37xx_chan_t, underrange), "%s.%s.%s.press-%d-underrange" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_em37xx_chan_t, error), "%s.%s.%s.press-%d-error" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_em37xx_chan_t, sync_err), "%s.%s.%s.press-%d-sync-err" },
  { HAL_S32, HAL_OUT, offsetof(lcec_em37xx_chan_t, raw_val), "%s.%s.%s.press-%d-raw" },
  { HAL_FLOAT, HAL_OUT, offsetof(lcec_em37xx_chan_t, val), "%s.%s.%s.press-%d-pressure" },
  { HAL_FLOAT, HAL_IO, offsetof(lcec_em37xx_chan_t, scale), "%s.%s.%s.press-%d-scale" },
  { HAL_FLOAT, HAL_IO, offsetof(lcec_em37xx_chan_t, bias), "%s.%s.%s.press-%d-bias" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};


static ec_pdo_entry_info_t lcec_em37xx_channel1[] = {
...
static ec_pdo_entry_info_t lcec_em37xx_channel2[] = {
...
static ec_pdo_info_t lcec_em37xx_pdos_in[] = {
    {0x1A00, 11, lcec_em37xx_channel1},
    {0x1A01, 11, lcec_em37xx_channel2},
};

static ec_sync_info_t lcec_em37xx_syncs[] = {
    {0, EC_DIR_OUTPUT, 0, NULL},
    {1, EC_DIR_INPUT,  0, NULL},
    {2, EC_DIR_OUTPUT, 0, NULL},
    {3, EC_DIR_INPUT,  2, lcec_em37xx_pdos_in},
    {0xff}
};
...
int lcec_em37xx_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  ...
  slave->proc_read = lcec_em37xx_read;

  // alloc hal memory
  if ((hal_data = hal_malloc(sizeof(lcec_em37xx_data_t))) == NULL) {
    rtapi_print_msg(RTAPI_MSG_ERR, LCEC_MSG_PFX "hal_malloc() for slave %s.%s failed\n", master->name, slave->name);
    return -EIO;
  }
  memset(hal_data, 0, sizeof(lcec_em37xx_data_t));
  slave->hal_data = hal_data;

  // initialize sync info
  slave->sync_info = lcec_em37xx_syncs;

  // initialize pins
  for (i=0; i<LCEC_EM37XX_CHANS; i++) {
